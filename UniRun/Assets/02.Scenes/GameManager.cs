...
public class GameManager : MonoBehaviour
{
    ...
    public Text scoerText; //점수를 출력할 UI텍스트
    ...
    private void Awake()
    {
        ...
        else
        {
            ...
            //씬에 두 개 이상의 이상의 GameManager 오브젝트가 조냊한다는 의
    ...
    public void AddScore(int newScore)
    {
        ...
        if(!isGameover)  // = if (isGameover) return;
        {
            //점수를 증가
            score += newScore;
            scoerText.text = "Score :" + score;

       
        }
    }
    //플레이어가 캐릭터 사망 시 게임오버를 실행하는 메서드
    public void OnPlayerDead()
    {
        isGameover = true;
        gameoverUI.SetActive(true); //소문자는 변수
    }
