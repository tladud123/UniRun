...
using UnityEngine;
//PlayerConteroller는 플레이어 캐릭터로서 Player 게임 오브젝트 제어함
...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && jumpCount < 2)
        {
            ...
            //합쳐져서 점프 높이가 비일관적으로 되는 현상을 막기위해서
    ...
    void Die()
    {
        ...
        //오디오 소스에 할당된 클립을 deathClipe으로 변경
    ...
    {
        //바닥에 닿자 마자 감지하는 처리
        //어떤 콜라이더랑 닿았으며, 충돌 표면이 위쪽을 보고있는지 확인
        if(collision.contacts[0].normal.y >0.7f)
        {
            //contact : 층돌 지점들의 정보를 담는 ContactPoint 타입의 데이터를
