...
{   //생성할 발판의 원본 프리팹
    ...
    void Update()
    {
        ...
        //마지막 배치 시첨에서 timeBetSpawn 이상 시간이 흘렀다면
        if (Time.time >= lastSpawnTime + timeBetSpawn)
        {
            //기록된 마지막 배치 시점을 현재 시점으로 갱신
            lastSpawnTime = Time.time;

            //다음 배치까지의 시간 간격을 timeBetSpawnMin,timeBetSpawnMax 사이에서 랜덤하게 가져오기
            timeBetSpawn = Random.Range(timeBetSpawnMin, timeBetSpawnMax);

            //배치할 위치의 높이를 yMin과 yMax 사이에서 랜덤하게 가져오기
            float yPos = Random.Range(yMin, yMax);

            //사용할 현재 순번의 발판 게임 오브젝트를 비활성하고 바로 즉시 다시 활성화
            //바로 즉시 다시 활성화 이 때, 발판의 Plarform 컴포넌트의 OnEnable 메서드가 실행됨
